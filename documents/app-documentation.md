...
- **Project URL:** `https://pgsqtbblahihqesnvwxd.supabase.co`
- **MCP Access:** Available via Supabase MCP for direct database management

**Client Configuration:**
- URL: `EXPO_PUBLIC_SUPABASE_URL` (configured in `eas.json`)
- Anon Key: `EXPO_PUBLIC_SUPABASE_ANON_KEY` (configured in `eas.json`)
...
- See `documents/supabase-database-documentation.md` for complete schema details, RLS policies, and RPC function documentation.
...
- **Project ID:** `pgsqtbblahihqesnvwxd`
- **Project URL:** `https://pgsqtbblahihqesnvwxd.supabase.co`
- **MCP Access:** Supabase MCP (Model Context Protocol) is configured and available for this project
  - Enables direct database management via MCP tools
  - Supports migrations, RPC functions, and schema management
  - Access to project logs, advisors, and edge functions

**EAS Project Details:**
- **EAS Project ID:** `2a4b5dbc-8e58-46c6-bd75-aadc7ac7dd49`
- Configured in `app.json` and `.eas/project.json`

### Environment Variables

**Required:**
- `EXPO_PUBLIC_SUPABASE_URL` - Supabase project URL (`https://pgsqtbblahihqesnvwxd.supabase.co`)
- `EXPO_PUBLIC_SUPABASE_ANON_KEY` - Supabase anonymous key (configured in `eas.json`)

**Optional:**
- `EXPO_PUBLIC_CONGRATS_ENDPOINT` - Congrats email endpoint (if configured)

**Note:** Environment variables are configured in `eas.json` for both `preview` and `production` build profiles.
